(() => {
  ...
  function initSplashCursor(options = {}) {
    const config = { ...defaults, ...options, PAUSED: false };
    ...
    function supportRenderTextureFormat(targetGl, internalFormat, format, type) {
      ...
      targetGl.framebufferTexture2D(
        targetGl.FRAMEBUFFER,
        targetGl.COLOR_ATTACHMENT0,
        targetGl.TEXTURE_2D,
        texture,
        0
      );
    ...
    function compileShader(type, source, keywords) {
      let shaderSource = addKeywords(source, keywords);
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => initSplashCursor(), { once: true });
  } else {
    initSplashCursor();
  }
})();